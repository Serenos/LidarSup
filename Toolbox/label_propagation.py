...
def labelPropagation(Mat_Label, Mat_Unlabel, labels, kernel_type = 'rbf', rbf_sigma = 1.5, \
                    knn_num_neighbors = 10, max_iter = 1000, tol = 1e-3):
    # initialize
    ...
    for i in range(num_unlabel_samples):
        label = np.argmax(label_function[i+num_label_samples])
        fg_pro, bg_pro = abs(label_function[i+num_label_samples][1]), abs(label_function[i+num_label_samples][0])
        fg_certainty = fg_pro/(bg_pro+fg_pro)
        unlabel_data_labels[i] = fg_certainty
